...
export default function WarehousePage({ role }) {
  ...
  const [catalogForm, setCatalogForm] = useState({ name: '', category: 'Tools', unit_cost: '', description: '' });
  const [assignForm, setAssignForm] = useState({ employee_id: '', equipment_catalog_id: '', serial_number: '', assigned_date: '', assigned_by: '', notes: '' });

  const WH_CATEGORIES = ['Tools', 'Uniform', 'Safety Equipment', 'Vehicle', 'Forklift', 'Radio', 'Badge/Access', 'Storage', 'Other Warehouse'];

  const load = useCallback(() => {
    fetch('/api/equipment/catalog').then(r => r.json()).then(setCatalog).catch(() => {});
    fetch('/api/equipment/assignments?department=Warehouse').then(r => r.json()).then(setAssignments).catch(() => {});
  ...
  const assignEquipment = async (e) => {
    ...
    await fetch('/api/equipment/assignments', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...assignForm, department: 'Warehouse' })
    });
  ...
  return (
    <div>
      <div className="section-header">
        <h2>Warehouse Equipment Management</h2>
        <div className="btn-group" style={{ margin: 0 }}>
          <button className="btn btn-primary" onClick={() => setShowAssignModal(true)}>+ Issue Equipment</button>
          <button className="btn btn-secondary" onClick={() => setShowCatalogModal(true)}>+ Add to Catalog</button>
        </div>
      </div>

      <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
        <button className={`btn btn-sm ${tab === 'assignments' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setTab('assignments')}>Issued Equipment</button>
        <button className={`btn btn-sm ${tab === 'catalog' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setTab('catalog')}>Equipment Catalog</button>
      </div>

      {tab === 'assignments' && (
        <div className="card">
          <h3>Warehouse Equipment Issued</h3>
          <div className="table-container">
            <table>
              <thead>
                <tr><th>Employee</th><th>Equipment</th><th>Serial #</th><th>Cost</th><th>Issued</th><th>Status</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {assignments.map(a => (
                  <tr key={a.id}>
                    ...
                    <td>{a.returned_date ? <span className="badge paid">Returned</span> : <span className="badge active">Issued</span>}</td>
                    <td>
                      {!a.returned_date && (
                ...
                {assignments.length === 0 && (
                  <tr><td colSpan="7" style={{ textAlign: 'center', color: '#888', padding: 30 }}>No warehouse equipment issued yet</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {tab === 'catalog' && (
        <div className="card">
          <h3>Warehouse Equipment Catalog</h3>
          <div className="table-container">
            <table>
              ...
              <tbody>
                ...
                {whCatalog.length === 0 && (
                  <tr><td colSpan="4" style={{ textAlign: 'center', color: '#888', padding: 30 }}>No warehouse catalog items yet</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {showCatalogModal && (
        <div className="modal-overlay" onClick={() => setShowCatalogModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>Add Warehouse Equipment to Catalog</h3>
              ...
              <div className="form-grid">
                ...
                <div className="form-group">
                  ...
                  <select value={catalogForm.category} onChange={e => setCatalogForm({...catalogForm, category: e.target.value})}>
                    {WH_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <div className="form-group">
      ...
      {showAssignModal && (
        <div className="modal-overlay" onClick={() => setShowAssignModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>Issue Warehouse Equipment</h3>
            <form onSubmit={assignEquipment}>
              <div className="form-grid">
                ...
                <div className="form-group">
                  ...
                  <select required value={assignForm.equipment_catalog_id} onChange={e => setAssignForm({...assignForm, equipment_catalog_id: e.target.value})}>
                    ...
                    {whCatalog.map(c => <option key={c.id} value={c.id}>{c.name} - ${c.unit_cost}</option>)}
                  </select>
                </div>
                <div className="form-group">
              ...
              <div className="btn-group">
                <button type="submit" className="btn btn-success">Issue Equipment</button>
